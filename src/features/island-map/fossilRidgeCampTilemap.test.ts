...
describe('buildFossilRidgeCampTileGrid', () => {
  ...
  it('has no colliding tiles beyond the world bounds', () => {
    expect(FOSSIL_RIDGE_CAMP_COLLIDING_TILES).toEqual([]);
  });

  it('places path tiles inside the harbor-exit rectangle, and nowhere else', () => {
    const grid = FOSSIL_RIDGE_CAMP_TILE_GRID;
