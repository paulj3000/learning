...
import { BoltsWorkshopScene, WORLD_HEIGHT, WORLD_WIDTH } from './scenes/BoltsWorkshopScene';
import {
  findInteraction,
...
/**
 * Bolt's Workshop's Phase 16 explorable environment (docs/ROADMAP.md
...
export function BoltsWorkshopWorldView({ childId, avatarKey }: BoltsWorkshopWorldViewProps) {
  ...
  const triggeredInteraction = triggeredInteractionId
    ? findInteraction(BOLTS_WORKSHOP_INTERACTIONS, triggeredInteractionId)
    : undefined;
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <Link className={styles.altNavLink} to={`/island/${childId}/locations/bolts-workshop`}>
