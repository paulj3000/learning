...
import { FossilRidgeCampWorldView } from './FossilRidgeCampWorldView';
...
vi.mock('./scenes/FossilRidgeCampScene', () => ({
  FossilRidgeCampScene: class {},
  WORLD_WIDTH: 960,
  WORLD_HEIGHT: 640,
}));
...
describe('FossilRidgeCampWorldView', () => {
  beforeEach(() => {
  ...
  it('shows a loading state before world changes resolve', () => {
    ...
    expect(screen.getByText(/loading fossil ridge camp/i)).toBeInTheDocument();
  });
  ...
  it('renders the scene and every always-available interaction once the story is complete', async () => {
    listAllWorldChangesMock.mockResolvedValue([
      { changeKey: 'DINOSAUR_EXPEDITION_COMPLETE' } as never,
    ]);

    renderWorldView();

    expect(await screen.findByTestId('phaser-game-container')).toBeInTheDocument();
    expect(screen.getByText('The assembled skeleton')).toBeInTheDocument();
    expect(screen.getByText('A set of dig tools')).toBeInTheDocument();
    expect(screen.getByText('The path back to Welcome Harbor')).toBeInTheDocument();
  });

  it('shows a plain message for tapping the skeleton instead of trying to start a session', async () => {
    const user = userEvent.setup();
    listAllWorldChangesMock.mockResolvedValue([
      { changeKey: 'DINOSAUR_EXPEDITION_COMPLETE' } as never,
    ]);
    ...
    renderWorldView();
    await user.click(await screen.findByText('The assembled skeleton'));

    expect(await screen.findByText(/fully assembled/i)).toBeInTheDocument();
  });
});
