...
import { StorykeeperCastleScene, WORLD_HEIGHT, WORLD_WIDTH } from './scenes/StorykeeperCastleScene';
import {
  findInteraction,
...
interface StorykeeperCastleWorldViewProps {
  childId: string;
  /** The child's already-chosen `ChildProfile.avatarKey`, resolved by the caller (`StorykeeperCastleWorldPage`). */
  avatarKey: string;
}

/**
 * Storykeeper Castle's Phase 14 explorable environment
 * (docs/LEARNING_ADVENTURE_ISLAND_EXPLORABLE_WORLD_ROADMAP.md section 33):
 * a physical creative-story environment. Deliberately parallel to
 * `WonderwildForestWorldView` rather than a shared component, same
...
export function StorykeeperCastleWorldView({
  ...
  const availableInteractions = useMemo(
    () =>
      context === null
        ? []
        : STORYKEEPER_CASTLE_INTERACTIONS.filter((interaction) =>
...
function buildGameConfig(
  ...
  return {
    ...
    scene: [new StorykeeperCastleScene(bus, interactionContext, avatarKey)],
  };
}

interface InteractionPanelProps {
  childId: string;
  interaction: (typeof STORYKEEPER_CASTLE_INTERACTIONS)[number];
  onDismiss: () => void;
  onDiscovered: () => void;
}
