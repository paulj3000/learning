...
This document covers nature/science adventure content only (Wonderwild
Forest per `docs/ROADMAP.md` Phase 6). It exists because science content
can be factually wrong in a way that story/counting content cannot: a
mis-stated fact is a credibility and trust problem for parents, not just a
gameplay bug.

## Why this is separate from general content authoring

Every other adventure (Pirate Builder Bay, Storykeeper Castle) is either
pure game logic (counting, ordering) or open-ended fiction with no factual
claim to get wrong. Wonderwild Forest adventures make real claims about
the natural world (what an animal does, why, how). Those claims need a
traceable source and a lightweight review step before they reach a child,
the same way a hint ladder needs a correctness check before it reaches the
engine.

## Authoring rule

Every `AdventureDefinition` under `wonderwild-forest` must carry a
`sources` doc comment directly above its `export const`, listing:
...
factual accuracy of a Wonderwild adventure is therefore fully determined
